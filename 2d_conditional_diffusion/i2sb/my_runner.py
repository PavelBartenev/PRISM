...
class MyRunner(object):
    ...
    def sample_batch(self, opt, loader, corrupt_method):
        if opt.corrupt == "mixture":
            ...
            clean_img, mask, y = next(loader)
            with torch.no_grad():
                corrupt_img = (1 - mask) * clean_img + mask * torch.randn_like(clean_img)
        else:
    ...
    def train(self, opt, train_dataset, corrupt_method=None):
        ...
        for it in range(opt.num_itr):
            ...
            if it % opt.save_model_iter == 0:
                print('Start saving model: ')
                if opt.global_rank == 0:
                    print('Global rank is zero: ')
                    torch.save({
