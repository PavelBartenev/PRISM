...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t, condition_tensors = None, noise = None):
        ...
        elif self.loss_type == 'l1_masked':
            loss = (noise - x_recon)[condition_tensors[:, [1], :, :, :] > 0].abs().mean()    
        else:
...
class Trainer(object):
    def __init__(
        ...
        self.step = 0
        self.lr_scheduler = torch.optim.lr_scheduler.LambdaLR(self.opt, self.lr_scheduler_func)
    ...
    def lr_scheduler_func(self, step):
        if self.step < 100_000:
            return 1
        elif 100_000 <= self.step <= 300_000:
            return 0.5
        elif self.step > 300_000:
            return 0.25

    def create_log_dir(self):
        now = datetime.datetime.now().strftime("%y-%m-%dT%H%M%S")
        log_dir = os.path.join(self.results_folder, now)
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            if new_lr != prev_lr:
                print(f"Learning rate changed from {prev_lr} to {new_lr}")
