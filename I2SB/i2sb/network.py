...
class Image256Net(torch.nn.Module):
    def __init__(self, log, noise_levels, use_fp16=False, cond=False, pretrained_adm=True, ckpt_dir="data/"):
        ...
        if pretrained_adm:
            ...
            #print('Before torch.load: ', psutil.cpu_percent(interval=None, percpu=True))
            out = torch.load(ckpt_pt, map_location="cpu")
            #print("After torch.load: ", psutil.cpu_percent(interval=None, percpu=True))
            self.diffusion_model.load_state_dict(out)
            print("After load.state_dict: ", psutil.cpu_percent(interval=None, percpu=True))
